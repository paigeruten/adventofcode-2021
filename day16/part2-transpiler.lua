...
local function transpile(packet)
  if packet.type_id == TYPE_IDS.LITERAL then
    return tostring(packet.literal)
  end

  local operands = table.map(packet.subpackets, transpile)
  ...
  elseif packet.type_id == TYPE_IDS.MINIMUM then
    return "math.min(" .. string.join(operands, ", ") .. ")"
  elseif packet.type_id == TYPE_IDS.MAXIMUM then
    return "math.max(" .. string.join(operands, ", ") .. ")"
...
local expr = transpile(packet)
print(expr)

local result = assert(load("return " .. expr))()
print(result)
